...
import { carListMock, carMock, userId } from "../__mocks__/car.mocks";
...
describe("Unit test: get one car", () => {
    test("get one car should work correctly", async () => {
        ...
        const data = await carServices.getOne(carMock.id, userId);
        ...
        expect(data?.userId).toBe(carListMock[0].userId);
    });
    test("get one car should work correctly", async () => {
        ...
        expect(data?.userId).toBe(carListMock[1].userId);
